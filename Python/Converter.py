...
# * Flip button to chng it decimal to binary and vice versa
     def flip(self):
          if self.state == 0:
               self.state = 1
     ...
     def convert(self, args):
          if self.state == 0:
               ...
               self.label.text = "In decimal it is:"
          else:
               val = bin(int(self.input.text))[2:]
               self.converted.text = val
               self.label.text = "In binary it is:"
...
# * Logo
          screen.add_widget(Image(
             source = "C:\VScode\JavaScript\images\logo.png",
